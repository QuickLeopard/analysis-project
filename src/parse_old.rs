...
fn alternative_from_n<Dest, A: Parser<Dest = Dest>>(alternatives: Vec<A>) -> Alt<Vec<A>> {
    Alt {
        parser: alternatives,
    }
}
...
mod test {
    ...
    #[test]
    fn test_just_parse() {
        let input = r#"UserCash{"user_id": "John", "count": 42,}"#.into();
        let result = just_parse::<UserCash>(input);
        assert_eq!(
            result,
            Ok((
                "".into(),
    ...
    fn test_alt_n() {
        ...
        assert_eq!(
            alt_n.parse("Hello World!".into()),
            Ok((" World!".into(), Status::Ok))
        );
    }
